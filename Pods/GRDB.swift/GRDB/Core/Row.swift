...
private struct StatementRowImpl: RowImpl {
    ...
    init(sqliteStatement: SQLiteStatement, statementRef: Unmanaged<SelectStatement>) {
        ...
        let keyDecodingStrategy = statementRef.takeUnretainedValue().database.configuration.keyDecodingStrategy
        let decodedColumnNames = (0..<sqlite3_column_count(sqliteStatement))
            .map { keyDecodingStrategy.decode(key: String(cString: sqlite3_column_name(sqliteStatement, Int32($0)))) }
