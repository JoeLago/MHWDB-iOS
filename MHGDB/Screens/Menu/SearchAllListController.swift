...
class SearchAllListController: DetailController {
    ...
    func updateTable(response: SearchResponse) {
        ...
        let palicoSection = addCustomSection(title: "Palico Weapons", data: response.palico, cellType: PalicoWeaponCell.self) { PalicoWeaponDetails(id: $0.id) }
        palicoSection.defaultCollapseCount = 5
        reloadData()
    }

    func addSearchSection<T: DetailCellModel>(data: [T], title: String? = nil, selectionBlock: ((T) -> UIViewController)? = nil) {
